...
if degree >= error: 
    log_f['error'].write('<br/>Rilevato %s°C, passato il range massimo: %s°C' % (
            degree, error))
elif degree >= warning: 
    log_f['warnint'].write('<br/>Rilevato %s°C, passato range di allerta: %s°C' % (
            degree, warning))
else:
    log_f['info'].write('<br/>Rilevato %s°C, nel range corretto < %s°C' % (
        degree, warning))
